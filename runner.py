...
#TODO: this breaks if theres a blank line in the prompt.
# Later the prompt should be a known input and we can use that to remove it more intelligently
def parse_response(output):
    ...
    for line in lines:
        ...
        if in_prompt and not capture_line:
            if line.strip() == "":
               capture_line = True
            continue
...
def run_llama(cmd):
    ...
    response = parse_response(llama_output)

    result = {
    "benchmark_time_sec": end - start,
    "prefill_speed_tps": prompt_tps,
    "decode_speed_tps": eval_tps,
    "prompt_tokens": prompt_tokens,
    "generated_tokens": eval_tokens,
    "inference_time_ms": total_ms,
    "total_tokens": total_tokens,

    "vram_total_mib": memory_data["total"],
    "vram_free_mib": memory_data["free"],
    "vram_used_mib": memory_data["total"] - memory_data["free"],
    "vram_model_mib": memory_data["model"],
    "vram_context_mib": memory_data["context"],
    "vram_compute_mib": memory_data["compute"],
    "vram_unaccounted_mib": memory_data["unaccounted"],
    "response": response,
    }
